...
class TestLiveTargets:
    ...
    def test_known_positives_via_message(self, sites_info, site, username):
        assert (
            simple_query(sites_info=sites_info, site=site, username=username)
            is QueryStatus.CLAIMED
        )
    ...
    def test_likely_negatives_via_message(self, sites_info, site, random_len):
        ...
        for i in range(num_attempts):
            ...
            random_handle = "".join(
                random.choice(acceptable_types) for _ in range(random_len)
            )
            attempted_usernames.append(random_handle)
            status = simple_query(
                sites_info=sites_info, site=site, username=random_handle
            )
            if status is QueryStatus.AVAILABLE:
                break
        assert (
            status is QueryStatus.AVAILABLE
        ), f"Could not validate available username after {num_attempts} attempts with randomly generated usernames {attempted_usernames}."
...
def test_username_illegal_regex(sites_info):
    ...
    assert (
        simple_query(sites_info=sites_info, site=site, username=invalid_handle)
        is QueryStatus.ILLEGAL
    )
