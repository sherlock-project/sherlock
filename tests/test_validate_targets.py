...
FALSE_POSITIVE_ATTEMPTS: int = (
    2  # Since the usernames are randomly generated, it's POSSIBLE that a real username can be hit
)
FALSE_POSITIVE_QUANTIFIER_UPPER_BOUND: int = (
    15  # If a pattern uses quantifiers such as `+` `*` or `{n,}`, limit the upper bound (0 to disable)
)
FALSE_POSITIVE_DEFAULT_PATTERN: str = (
    r"^[a-zA-Z0-9]{7,20}$"  # Used in absence of a regexCheck entry
)


def set_pattern_upper_bound(
    pattern: str, upper_bound: int = FALSE_POSITIVE_QUANTIFIER_UPPER_BOUND
) -> str:
    ...
    pattern = re.sub(
        r"(?<!\\)\{(\d+),\}", replace_upper_bound, pattern
    )  # {n,} # type: ignore
...
def false_positive_check(
    sites_info: dict[str, dict[str, str]], site: str, pattern: str
) -> QueryStatus:
    ...
    for _ in range(FALSE_POSITIVE_ATTEMPTS):
        ...
        if not hasattr(result, "status"):
            raise TypeError(
                f"Result for site {site} does not have 'status' attribute. Actual result: {result}"
            )
...
def false_negative_check(
    sites_info: dict[str, dict[str, str]], site: str
) -> QueryStatus:
...
class Test_All_Targets:
    ...
    def test_false_pos(self, chunked_sites: dict[str, dict[str, str]]):
        ...
        for site in chunked_sites:
            ...
            assert (
                result is QueryStatus.AVAILABLE
            ), f"{site} produced false positive with pattern {pattern}, result was {result}"
    ...
    def test_false_neg(self, chunked_sites: dict[str, dict[str, str]]):
        ...
        for site in chunked_sites:
            ...
            assert (
                result is QueryStatus.CLAIMED
            ), f"{site} produced false negative, result was {result}"
